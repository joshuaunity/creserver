...
app.get('/owners/:id/car/', async (req, res) => {
    try {
        const ownerId = req.params.id;

        // Find the owner by ID and populate the 'car' field
        const owner = await Owner.findById(ownerId).populate('car');
        ...
        // If owner exists, fetch the car and respond with it
        const car = await Car.findById(owner.car);
        ...
        res.status(500).json({
            error: "An error occurred while fetching the owner's car"
        });
    }
});
